...
class TestHaversine:
    ...
    def test_antipode_is_pi_R(self) -> None:
        ...
        assert float(d) == pytest.approx(np.pi * EARTH_RADIUS_KM, rel=1e-4)

    def test_known_route_ny_to_london(self) -> None:
        d = haversine_distance(40.7128, -74.0060, 51.5074, -0.1278)
        assert float(d) == pytest.approx(5570.0, abs=20.0)
    ...
    def test_broadcasts(self) -> None:
        ...
        np.testing.assert_allclose(
            d, np.full(2, np.pi / 2 * EARTH_RADIUS_KM), rtol=1e-4
        )


class TestGaspariCohn:
    def test_one_at_zero(self) -> None:
        assert float(gaspari_cohn(np.array([0.0]), 1000.0)) == pytest.approx(1.0)
    ...
    def test_continuous_at_radius_boundary(self) -> None:
        ...
        assert float(np.abs(below - above)) < 1e-7
